...
export function Header()
{
    ...
    return(
        <>
        <div>
            <ul className={`${menu?"flex flex-col gap-4 items-start text-white font-[Quicksand]":"items-center flex flex-col gap-4  text-white font-[Quicksand]"}`}>
                <li className="flex gap-3 justify-center items-center cursor-pointer hover:font-bold group"><span><HiOutlineRectangleGroup size={`${menu?25:28}`} className="font-bold  group-hover:scale-110 group-hover:fill-white"/></span><a className={`${menu ? "text-md":"hidden"}`}>Dashboard</a></li>
                <li className="flex gap-3 justify-center items-center cursor-pointer hover:font-bold group"><span>< PiBuildingApartmentLight size={`${menu?25:28}`} className="font-bold group-hover:scale-110 group-hover:fill-white"/></span><a className={`${menu ? "text-md  ":"hidden "}`}>Components</a></li>
